  ...
  return (
    <div className="p-10 max-w-5xl">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="font-display text-4xl leading-none tracking-wide text-accent mb-1">Receive Invoice</h1>
          <p className="text-text-secondary text-sm">
            Reconcile against <span className="font-medium text-text-primary">{po.supplier}</span>
        ...
        {results && !syncDone && (
          <button
            ...
            disabled={syncing}
            className="inline-flex items-center gap-2 bg-accent hover:bg-accent-dim disabled:opacity-60 text-white text-sm font-semibold px-5 py-2.5 rounded transition-colors"
          >
      ...
      {!results && (
        <div
          ...
          className={`rounded-lg border-2 border-dashed py-16 text-center transition-colors cursor-pointer mb-6 ${
            parsing ? "border-border-1 bg-surface-2 cursor-default"
              : dragging ? "border-brand-green bg-surface-2"
              : "border-border-1 bg-surface-1 hover:border-accent hover:bg-surface-2"
          }`}
        >
          ...
          {parsing ? (
            <div className="flex flex-col items-center gap-3">
              <div className="w-8 h-8 border-4 border-border-1 border-t-accent rounded-full animate-spin" />
              <p className="text-accent font-medium">Parsing invoice with AI…</p>
              {filename && <p className="text-sm text-text-tertiary">{filename}</p>}
            </div>
          ) : (
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-accent font-semibold">Drop the received invoice here</p>
              <p className="text-text-tertiary text-sm">PDF, PNG, or JPEG · AI will match each line to the original PO</p>
            </div>
          )}
        </div>
      )}
      ...
      {results && (
        <>
          ...
          <div className="flex items-center gap-4 mb-4 text-sm">
            <span className="text-text-secondary"><span className="font-semibold text-text-primary">{results.length}</span> lines compared</span>
            ...
            <button onClick={() => { setResults(null); setFilename(null); }} className="ml-auto text-xs text-text-tertiary hover:text-text-secondary underline">Upload different invoice</button>
          </div>

          <div className="bg-surface-1 rounded-lg border border-border-1 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-[11px] text-text-tertiary uppercase tracking-widest border-b border-border-0 bg-surface-1">
                ...
                <tbody>
                  {results.map((r, i) => (
                    <tr key={i} className={`border-b border-border-0 last:border-0 hover:bg-surface-1/60 ${r.status !== "EXACT" ? "bg-amber-50/30" : ""}`}>
                      <td className="px-5 py-3">
                        <p className="font-medium text-text-primary">{r.name}</p>
                        <p className="text-[11px] text-text-tertiary font-mono">{r.sku || <span className="italic">no SKU</span>}</p>
                      </td>
                      <td className="px-5 py-3 text-center"><StatusBadge status={r.status} /></td>
                      <td className="px-5 py-3 text-right text-text-secondary">{r.expectedQty || "—"}</td>
                      <td className="px-5 py-3 text-right font-semibold text-text-primary">{r.actualQty || "—"}</td>
                      ...
                      <td className="px-5 py-3 text-right text-text-secondary">{r.expectedCost > 0 ? `$${r.expectedCost.toFixed(2)}` : "—"}</td>
                      <td className="px-5 py-3 text-right text-text-primary">{r.actualCost > 0 ? `$${r.actualCost.toFixed(2)}` : "—"}</td>
